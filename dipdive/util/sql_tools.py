...
def insert_row(email, name,age, city, password, user_name=config.mysql_user, mysql_password=config.mysql_password, host_name=config.mysql_host, db_name=config.db_name, table_name=config.table_name):
    # Define our connection string
    conn = MySQLdb.connect(user=user_name, password=mysql_password, host=host_name, database=db_name)
    try:
        cursor = conn.cursor()
        cursor.execute("""insert into %s (email, name, password, age, city) values ('%s', '%s', '%s', '%s', '%s'); """ %(table_name, email, name, password, age, city))
        conn.commit()
    finally:
        conn.close()
    print('Row inserted')

def update_row(email, name, mobile, age, city, user_name=config.mysql_user, host_name=config.mysql_host, db_name=config.db_name, table_name=config.table_name):
    # Define our connection string
    conn = MySQLdb.connect(user=user_name, host=host_name, database=db_name)
    try:
        cursor = conn.cursor()
        cursor.execute("""update %s set name='%s', mobile='%s', age='%s', city='%s' where email='%s'; """ %(table_name, email, name, mobile, age, city))
